...
class ScoringAttentionModule(nn.Module):
    ...
    def forward(self, embedding_1, embedding_2, nframes_2):
        # print(embedding_1.size(), embedding_2.size())

        att = torch.bmm(embedding_1, embedding_2)#.squeeze(2)# / (torch.norm(aud_em, dim=1) * torch.norm(image_embedding, dim=1))
        s, _ = att.max(dim=1)
        # print(att.size(), s.size())
        return s 
