...
def validate(audio_model, image_model, attention, contrastive_loss, val_loader, rank, image_base, args):
    ...
    with torch.no_grad():
        ...
        for value_dict in tqdm(val_loader, leave=False):
            ...
            anch_s = attention(image_output.transpose(1, 2), english_output, english_nframes).unsqueeze(-1)
            anch.append(anch_s)
            ...
            for neg_dict in value_dict['negatives']:
                ...
                score = attention(neg_image_output.transpose(1, 2), english_output, english_nframes).unsqueeze(-1)
                ns.append(score)
    ...
    if args["resume"]:
        ...
        if "restore_epoch" in args:
            info, start_epoch, global_step, best_epoch, best_acc = loadModelAttriburesAndTrainingAtEpochAMP(
                args["exp_dir"], english_model, image_model, attention, contrastive_loss, optimizer, rank, args["restore_epoch"]
                )
    ...
    for epoch in np.arange(start_epoch, args["n_epochs"] + 1):
        ...
        for value_dict in train_loader:
            ...
            with torch.cuda.amp.autocast():
                ...
                image_output = image_model(value_dict['image'].to(rank))
                english_input = value_dict["english_feat"].to(rank)
                _, _, english_output = english_model(english_input)
                english_nframes = NFrames(english_input, english_output, value_dict["english_nframes"]) 
                ...
                #     _, _, pos_english_output = english_model(pos_english_input)
                ...
                #     _, _, neg_english_output = english_model(neg_english_input)
                ...
                loss = compute_matchmap_similarity_matrix_loss(
                    image_output, 
                    english_output, english_nframes,
                    # negatives, positives, 
                    attention, contrastive_loss, #audio_attention,  
