...
with torch.no_grad():
    ...
    for ep_num in tqdm(episodes):
        ...
        for i in range(query_output.size(0)):
            label_a = query_labels[ep_num][i]
            for j in range(query_output.size(0)):
                label_b = query_labels[ep_num][j]
                if label_a == label_b: continue
            
                im = torch.cat([query_image[i, :, :].unsqueeze(0), query_image[j, :, :].unsqueeze(0)], dim=0)
                labels = [label_a, label_b]
                scores = attention.one_to_many_score(im, query_output[i, :, :].unsqueeze(0)).squeeze()
...
# for w in per_novel_word:
#     for w_2 in per_novel_word[w]:
#         d = per_novel_word[w][w_2]
#         c = d['count']
#         t = d['encountered']
#         # print(w)
#         if t == 0: p = 0
#         else: p = round(100*c/t, 2)
#         print(f'{w:<12} {w_2:<12} {c}/{t}={p:.2f}%')
#         f.write(f'{w:<12} {w_2:<12} {p}\n')