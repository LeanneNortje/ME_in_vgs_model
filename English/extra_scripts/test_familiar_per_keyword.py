    ...
    for ep_num in tqdm(episodes):
        ...
        for i in range(query_output.size(0)):
            ...
            if tags_1[i] == 'familiar_1':
                label = labels_1[i]
                if label not in totals: totals[label] = 0
                if label not in per_word: 
                    per_word[label] = {}
                    for l, _, _, _ in episodes[0]['familiar_test']['test']: 
                        per_word[label][l] = {'chosen': 0, 'counting': 0}
                    per_word[label][label] = {'chosen': 0, 'counting': 0}

                per_word[label][labels[index]]['chosen'] += 1
                per_word[label][labels[0]]['counting'] += 1
                per_word[label][labels[1]]['counting'] += 1
                totals[label] += 1
...
with open(Path('results/familiar_keyword_analysis.csv'), 'wt') as out_file:
