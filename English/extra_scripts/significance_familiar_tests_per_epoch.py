            ...
            for ep_num in tqdm(query, desc=f'{model_instance}'):
                ...
                for i in range(query_output.size(0)):
                    ...
                    if trial not in significance_dict:
                        ...
                        if significance_dict[trial]['epoch'] != epoch or significance_dict[trial]['query'] != query_audio_samples[i] or significance_dict[trial]['image_A'] != query_image_samples[i] and significance_dict[trial]['image_B'] != other_image_samples[i]:
                            print('mismatch A')
                    ...
                    if trial not in significance_dict: 
                        ...
                        if significance_dict[trial]['epoch'] != epoch or significance_dict[trial]['query'] != secondary_audio_samples[i] or significance_dict[trial]['image_B'] != query_image_samples[i] and significance_dict[trial]['image_A'] != other_image_samples[i]:
                            print('mismatch B')
...
trial = 1
for epoch in tqdm(range(1, args['n_epochs']+1), desc=f'{model_instance}'):
    for ep_num in query:
...
with open('results/significance_familiar_per_epoch.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    writer.writerow(header)
    for trial in tqdm(significance_dict, desc=f'Writing to csv'):
        entry = significance_dict[trial]
        row = [trial, entry['query'], entry['image_A'], entry['image_B'], entry['epoch'], entry['none'], entry['alexnet'], entry['cpc'], entry['cpc+alexnet'], entry['random_baseline']]
        writer.writerow(row)
