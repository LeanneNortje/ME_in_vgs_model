...
with torch.no_grad():
    ...
    results = {}
    familiar_results = {}
    novel_classes_considered = ['nautilus', 'chair', 'bus', 'butterfly', 'cannon', 'bench']
    for novel_class in novel_classes_considered:
        print(novel_class)
    ...
    # for familiar_class in familiars:
    #     print(familiar_class)

    #     if familiar_class not in familiar_results: results[familiar_class] = {}
    #     if familiar_class not in familiar_results[familiar_class]: familiar_results[familiar_class][familiar_class] = []

    #     for i1, wav1 in enumerate(tqdm(familiars[familiar_class], desc='Within class')):
    #         for i2, wav2 in enumerate(familiars[familiar_class]):
    #             if i1 != i2:

    #                 output1 = familiar_embeddings[wav1]
    #                 output2 = familiar_embeddings[wav2]
    #                 s = attention.one_to_many_score(output1, output2).squeeze()
                    
    #                 familiar_results[familiar_class][familiar_class].append(s.item())



    #     for i1, wav1 in enumerate(tqdm(familiars[familiar_class], desc='Other familiar classes')):
    #         for familiar_class1 in familiars:
    #             if familiar_class != familiar_class1: 

    #                 if familiar_class1 not in familiar_results[familiar_class]: familiar_results[familiar_class][familiar_class1] = []
    #                 for i2, wav2 in enumerate(familiars[familiar_class1]):

    #                     output1 = familiar_embeddings[wav1]
    #                     output2 = familiar_embeddings[wav2]
    #                     s = attention.one_to_many_score(output1, output2).squeeze()
                        
    #                     familiar_results[familiar_class][familiar_class1].append(s.item())

    #     for i1, wav1 in enumerate(tqdm(familiars[familiar_class], desc='Other familiar classes')):
    #         for novel_class in novel_classes_considered:

    #             if novel_class not in familiar_results[familiar_class]: familiar_results[familiar_class][novel_class] = []

    #             for i2, wav2 in enumerate(novels[novel_class]):

    #                 novel_output = novel_embeddings[wav2]
    #                 familiar_output = familiar_embeddings[wav1]
    #                 s = attention.one_to_many_score(novel_output, familiar_output).squeeze()

    #                 familiar_results[familiar_class][novel_class].append(s.item())
...
# print('Familiar ****************************')
# for familiar_class in familiar_results:
#     for other_class in familiar_results[familiar_class]:

#         s = np.mean(familiar_results[familiar_class][other_class])
#         print(familiar_class, other_class, s)

#         f.write(f'{familiar_class:<12} {other_class:<12} {s}\n')