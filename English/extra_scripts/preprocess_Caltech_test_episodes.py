...
#         # Novel
        ...
        audio_1[ep_num] = []
        audio_2[ep_num] = []
        audio_labels_1[ep_num] = []
        audio_labels_2[ep_num] = []
        audio_tag_1[ep_num] = []
        audio_tag_2[ep_num] = []

        for test_class, imgpath, wav in episode['novel_test']['novel']:
            ...
            if wav not in audio_datapoints:
                ...
                audio_datapoints[wav] = this_english_audio_feat
            audio_labels_1[ep_num].append(test_class)
            audio_1[ep_num].append(wav)
            audio_tag_1[ep_num].append('novel')

        for known_class, imgpath, wav in episode['novel_test']['known']:
            ...
            if wav not in audio_datapoints:
                ...
                this_english_audio_feat, this_english_nframes = PadFeat(this_english_audio_feat, target_length, padval)
                audio_datapoints[wav] = this_english_audio_feat
            audio_labels_2[ep_num].append(known_class)
            audio_2[ep_num].append(wav)  
            audio_tag_2[ep_num].append('known_novel')


        for test_class, imgpath, wav in episode['familiar_test']['test']:
            if imgpath not in image_datapoints:
                this_image = LoadImage(imgpath, resize, image_normalize, to_tensor)
                image_datapoints[imgpath] = this_image.unsqueeze(0)
            image_labels_1[ep_num].append(test_class)
            ...
            audio_labels_1[ep_num].append(test_class)
            audio_1[ep_num].append(wav)
            audio_tag_1[ep_num].append('familiar_1')

        for known_class, imgpath, wav in episode['familiar_test']['other']:
            ...
            audio_labels_2[ep_num].append(known_class)
            audio_2[ep_num].append(wav)  
            audio_tag_2[ep_num].append('familiar_2')

save_fn = Path('results/Caltech_episode_data')
...
np.savez(
    ...
    audio_1=audio_1,
    audio_labels_1=audio_labels_1,
    audio_tag_1=audio_tag_1,
    audio_2=audio_2,
    audio_labels_2=audio_labels_2,
    audio_tag_2=audio_tag_2,
    image_1=image_1,
