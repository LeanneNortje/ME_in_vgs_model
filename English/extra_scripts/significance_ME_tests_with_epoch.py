        ...
        with torch.no_grad():
            for ep_num in tqdm(query, desc=f'{model_instance}'):
                ...
                for i in range(query_output.size(0)):
                    ...
                    if epoch not in significance_dict[ep_num]: 
                        significance_dict[ep_num][epoch] = {}

                    if query_audio_samples[i] not in significance_dict[ep_num][epoch]: 

                        significance_dict[ep_num][epoch][query_audio_samples[i]] =  {
                            'image_A': query_image_samples[i],
                            'image_B': other_image_samples[i]
                        }

                    if model_instance in significance_dict[ep_num][epoch][query_audio_samples[i]]: print('PROBLEM')
                    significance_dict[ep_num][epoch][query_audio_samples[i]][model_instance] = int(index==0)
...
for ep_num in tqdm(query, desc=f'{model_instance}'):
    ...
    for epoch in range(1, args['n_epochs']+1):
        ...
        if ep_num not in significance_dict: significance_dict[ep_num] = {}
        if epoch not in significance_dict[ep_num]: significance_dict[ep_num][epoch] = {}
        episode = episodes[ep_num]
        tags_1 = query_tag[ep_num]
        labels_1 = query_labels[ep_num]
        tags_2 = secondary_tag[ep_num]
...
header = ['trial', 'audio_query', 'image_A', 'image_B', 'epoch', 'none', 'alexnet', 'cpc', 'cpc+alexnet', 'random_baseline']
trial = 1
with open('results/significance_per_epoch.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    writer.writerow(header)
    for num in tqdm(significance_dict, desc=f'Writing to csv'):
        for epoch in significance_dict[num]:
            for query in significance_dict[num][epoch]:
                entry = significance_dict[num][epoch][query]
                row = [trial, query, entry['image_A'], entry['image_B'], epoch, entry['none'], entry['alexnet'], entry['cpc'], entry['cpc+alexnet'], entry['random_baseline']]
                writer.writerow(row)
                trial += 1