...
episodes = np.load(Path('data/Caltech_episodes.npz'), allow_pickle=True)['episodes'].item()
...
with torch.no_grad():
    ...
    audio_datapoints = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['audio_datapoints'].item()
    image_datapoints = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['image_datapoints'].item()

    query = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['audio_1'].item()
    query_labels = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['audio_labels_1'].item()
    query_tag = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['audio_tag_1'].item()

    secondary_query = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['audio_2'].item()
    secondary_labels = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['audio_labels_2'].item()
    secondary_tag = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['audio_tag_2'].item()

    image_1 = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['image_1'].item()
    image_2 = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['image_2'].item()
    image_labels_1 = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['image_labels_1'].item()
    image_labels_2 = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['image_labels_2'].item()
    image_tag_1 = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['image_tag_1'].item()
    image_tag_2 = np.load(Path('results/Caltech_episode_data.npz'), allow_pickle=True)['image_tag_2'].item()
    ...
    for ep_num in tqdm(episodes, desc=f'{rewind}'):
        ...
        for label in query_labels[ep_num]:
            if label not in per_novel_word: per_novel_word[label] = {'correct': 0, 'total': 0}
        ...
        for i in range(query_a.size(0)):
            ...
            if index == 0: 
                ...
                per_novel_word[query_labels[ep_num][a[i]]]['correct'] += 1
                c += 1
            novel_results['total'] += 1
            per_novel_word[query_labels[ep_num][a[i]]]['total'] += 1
            t += 1
            ...
            if index == 1: 
                novel_results['correct'] += 1
                per_novel_word[query_labels[ep_num][b[i]]]['correct'] += 1
                c += 1
            novel_results['total'] += 1
            per_novel_word[query_labels[ep_num][b[i]]]['total'] += 1
            t += 1
        print(f'{c}/{t}={100*c/t}%')
