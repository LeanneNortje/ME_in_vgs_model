...
#             _, _, secondary_query_output = english_model(secondary_query_output.to(rank))
        ...
        for i in range(query_output.size(0)):
            ...
            # print(images.size(), query_output[i, :, :].unsqueeze(0).size(), scores.size())
            # print(scores)
            # print(scores)
            # print(attention(query_image[i, :, :].unsqueeze(0), query_output[i, :, :].unsqueeze(0)))
            # print(attention(other_image[i, :, :].unsqueeze(0), query_output[i, :, :].unsqueeze(0)))
            ...
            distance_1 = cos(im.squeeze().unsqueeze(0), query_output[i, :, :].squeeze().unsqueeze(0)).item()
            ...
            distance_2 = cos(im.squeeze().unsqueeze(0), query_output[i, :, :].squeeze().unsqueeze(0)).item()
