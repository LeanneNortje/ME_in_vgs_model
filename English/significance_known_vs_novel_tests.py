...
for args_fn in Path('model_metadata').rglob('*/args.pkl'):
    ...
    with torch.no_grad():
        trial = 1
        for ep_num in tqdm(sorted(query), desc=f'{model_instance}'):
            ...
            for i in range(secondary_query_output.size(0)):
                ...
                if model_instance in significance_dict[trial]: print('PROBLEM', secondary_audio_samples[i])
...
for ep_num in tqdm(query, desc=f'{model_instance}'):
    ...
    for i in range(len(secondary_audio_samples)):
...
with open('results/significance_known_vs_novel.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
