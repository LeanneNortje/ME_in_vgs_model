...
with torch.no_grad():
    ...
    for ep_num in tqdm(episodes):
        ...
        indices.pop(0)
        for i in range(len(image_tag_1[ep_num])):
            if image_tag_1[ep_num][i] == 'familiar_1': indices.append(i)
...
with open(Path(f'results/files/mismatched_novel_keyword_analysis{name}.csv'), 'wt') as out_file:
