...
for args_fn in Path('model_metadata').rglob('*/args.pkl'):
    ...
    with torch.no_grad():
        for ep_num in tqdm(query, desc=f'{model_instance}'):
            ...
            indices = [i for i in range(len(image_tag_1[ep_num])) if image_tag_1[ep_num][i] == 'novel']
            indices.append(indices[0])
            indices.pop(0)

            temp = query_image[indices]
            query_image = temp
            image_labels = [image_labels_1[ep_num][lab] for lab in indices]
    ...
    #             _, _, secondary_query_output = english_model(secondary_query_output.to(rank))
            ...
            for i in range(query_output.size(0)):
                if labels_1[i] == image_labels[i]: print(image_labels[i])
                ...
                if model_instance in significance_dict[ep_num][query_audio_samples[i]]: print('PROBLEM')
                significance_dict[ep_num][query_audio_samples[i]][model_instance] = int(index==0)
...
trial = 1
with open('results/significance_mismatched_ME.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    ...
    for num in tqdm(significance_dict, desc=f'Writing to csv'):
        for query in significance_dict[num]:
            ...
            row = [trial, query, entry['image_A'], entry['image_B'], entry['none'], entry['alexnet'], entry['cpc'], entry['cpc+alexnet'], entry['random_baseline']]
            writer.writerow(row)
            trial += 1
